 ...
 * @see DbBaseInterface
 */
require CORE_PATH . 'modules/extensions/db/db_base_interface.php';

/**
 * @see DbLoader
 */
require CORE_PATH . 'modules/extensions/db/loader/loader.php';
/**
* @see ActiveRecordBase
*/
require CORE_PATH . 'modules/extensions/db/active_record_base/active_record_base.php';
	...
	public static function raw_connect($new_connection=false, $database = null){
		...
		if(isset($config['pdo'])){
			...
			if(!class_exists($dbclass)) {
				 ...
				 * @see DbPDO
				 */
				require_once CORE_PATH . 'modules/extensions/db/adapters/pdo.php';
				require_once CORE_PATH . 'modules/extensions/db/adapters/pdo/' . $config['type'] . '.php';
			}
		} else {
			$dbclass = "Db{$config['type']}";
			if(!class_exists($dbclass)) {
				require_once CORE_PATH . 'modules/extensions/db/adapters/' . $config['type'] . '.php';
			}
		}
		if(!class_exists($dbclass)){
